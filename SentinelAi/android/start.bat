...
cd /d "%PROJECT_DIR%"
set "APP_ID=com.example.sentinelai"
set "MAIN_ACTIVITY=com.example.sentinelai.MainActivity"
set "GRADLEW=%PROJECT_DIR%\gradlew.bat"
set "APK_PATH=%PROJECT_DIR%\app\build\outputs\apk\debug\app-debug.apk"
set "APK_DEST=%PROJECT_DIR%\..\SentinelAI.apk"
set "DRY_RUN=0"
set "AVD_NAME="
...
if "%DRY_RUN%"=="1" (
  ...
  echo [DRY-RUN] copy "%APK_PATH%" "%APK_DEST%"
) else (
  ...
  echo [INFO] Aggiorno SentinelAI.apk nella root del progetto...
  copy /Y "%APK_PATH%" "%APK_DEST%" >nul
  echo [OK] APK aggiornato: %APK_DEST%
