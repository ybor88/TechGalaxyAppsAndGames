...
title Contabilita Industriale 2.0 — Avvio

echo.
echo  ╔══════════════════════════════════════════════════════╗
echo  ║         Contabilita Industriale 2.0  —  Avvio completo         ║
echo  ╚══════════════════════════════════════════════════════╝
echo.
...
set DB_FILE=%ROOT%backend\contabilitaindustriale20.db
...
:: Termina eventuale uvicorn rimasto dalla sessione precedente (porta 8001)
...
if exist "%DB_FILE%" (
    "%PYTHON%" -c "import sqlite3; db=sqlite3.connect(r'%DB_FILE%'); db.execute('PRAGMA journal_mode=WAL'); db.execute('PRAGMA busy_timeout=30010'); db.execute('PRAGMA synchronous=NORMAL'); db.commit(); db.close(); print('    WAL mode, busy_timeout=30s, synchronous=NORMAL applicati.')"
    if errorlevel 1 (
...
:: Backend
start "Contabilita Industriale 2.0 — Backend (porta 8001)" cmd /k "cd /d %BACKEND% && call .venv\Scripts\activate.bat && uvicorn app.main:app --host 127.0.0.1 --port 8001"

:: Attende che il backend sia in ascolto (max 40 s)
echo     Attendo avvio backend (porta 8001)...
set /a BACK_WAIT=0
:backend_wait_loop
timeout /t 2 /nobreak >nul
...
:backend_ready
echo     Backend attivo sulla porta 8001.

:backend_started

:: Frontend
start "Contabilita Industriale 2.0 — Frontend (porta 3001)" cmd /k "cd /d %FRONTEND% && npm start"

:: Attende che il frontend sia in ascolto (max 40 s)
echo     Attendo avvio frontend (porta 3001)...
set /a FRONT_WAIT=0
:frontend_wait_loop
timeout /t 2 /nobreak >nul
...
:frontend_ready
echo     Frontend attivo sulla porta 3001.
...
echo.
echo  ╔══════════════════════════════════════════════════════╗
echo  ║  Contabilita Industriale 2.0 in esecuzione                      ║
echo  ║                                                      ║
